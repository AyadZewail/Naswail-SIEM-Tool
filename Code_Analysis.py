    ...
    def get_location(self, ip):
        try:
            with geoip2.database.Reader(self.geoip_db_path) as reader:
                ...
                country = response.country.iso_code  # Get country ISO code
        ...
        try:
            ...
            for i in range(self.lastindex, len(self.packets)):
                if IP in self.packets[i]:
                    ...
                    src_lat, src_lon, src_country = self.get_location(self.packets[i][IP].src)
                    dst_lat, dst_lon, dst_country = self.get_location(self.packets[i][IP].dst)
            ...
            # Set the pixmap to the label in your UI
